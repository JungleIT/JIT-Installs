...
#     installed channel. That's its job — don't reinvent it by scraping
#     Microsoft Learn HTML or parsing channel GUIDs out of registry.
#   - We just snapshot the installed build, kick off /Update, wait for the
#     C2R worker processes to exit (deterministic finish signal), then
#     diff the build. Three outcomes:
...
Start-Sleep -Seconds 10   # let workers spawn
